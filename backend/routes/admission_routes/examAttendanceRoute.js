...
const attendanceQrDir = path.join(__dirname, "..", "..", "uploads", "AttendanceQrCodes");
if (!fs.existsSync(attendanceQrDir)) fs.mkdirSync(attendanceQrDir, { recursive: true });

const formatActorRole = (role) => {
  const safeRole = String(role || "proctor").trim();
  return safeRole.split(/[\s_-]+/).map((p) => p.charAt(0).toUpperCase() + p.slice(1).toLowerCase()).join(" ");
};
...
router.get("/exam-attendance/token/:applicant_number", async (req, res) => {
  ...
  try {
    ...
    if (!assigned) return res.status(404).json({ error: "No confirmed schedule yet." });
    ...
    if (!record) return res.status(404).json({ error: "QR not generated yet." });
...
router.post("/exam-attendance/generate/:schedule_id", async (req, res) => {
  ...
  try {
    ...
    for (const row of applicants) {
      ...
      await QRCode.toFile(path.join(attendanceQrDir, qrFilename), token, { width: 300 });
      results.push({ applicant_id, qr_token: token, qr_url: `/uploads/AttendanceQrCodes/${qrFilename}` });
    }
    res.json({ success: true, generated: results });
  } catch (err) {
    console.error("Error generating attendance QR codes:", err);
    res.status(500).json({ success: false, error: "Failed to generate attendance QR codes." });
  }
});
...
router.post("/exam-attendance/scan", async (req, res) => {
  ...
  if (!token) return res.status(400).json({ success: false, message: "No QR token provided." });
  ...
  try {
    ...
    if (record.status === "present") {
      return res.status(409).json({
        ...
        applicant: { applicant_id: record.applicant_id, name: `${record.last_name}, ${record.first_name} ${record.middle_name || ""}`.trim() },
      });
    }
    ...
    if (result.affectedRows === 0) {
      return res.status(409).json({ success: false, message: "This QR code was already scanned." });
    }

    const { actorId, actorRole } = resolveAuditActor(req);
    const applicantName = `${record.last_name}, ${record.first_name} ${record.middle_name || ""}`.trim();
    ...
    res.status(500).json({ success: false, message: "Server error while scanning." });
  }
});

// Bulk mark remaining as absent
router.put("/exam-attendance/mark-absent/:schedule_id", async (req, res) => {
  const { schedule_id } = req.params;
  const { audit_actor_id, audit_actor_role } = req.body;
  try {
    const [result] = await db.query(
      `UPDATE exam_attendance SET status='absent' WHERE schedule_id=? AND status='not_arrived'`,
      [schedule_id],
    );
    await insertAuditLogAdmission({
      ...
      severity: "INFO",
      message: `${formatActorRole(audit_actor_role)} (${audit_actor_id || "unknown"}) marked ${result.affectedRows} applicant(s) ABSENT for schedule ${schedule_id}.`,
    });
    res.json({ success: true, marked_absent: result.affectedRows });
  } catch (err) {
    console.error("Error marking absentees:", err);
    res.status(500).json({ success: false, error: "Failed to mark absentees." });
  }
});
