  ...
  const headerColor = colors.header || "#1976d2";
    ...
    const fetchGwaPrintingStatus = async () => {
        try {
            ...
            setGwaPrintStatus({ overall: false, per_semester: false, loading: false });
        }
    };

    useEffect(() => {
        fetchGwaPrintingStatus();
    }, []);

    const handleToggleGwaPrinting = async (gwaType, nextChecked) => {
        if (!canEdit) {
            setSnackbar({
                open: true,
                message: "You do not have permission to change GWA printing settings.",
                severity: "warning",
            });
            return;
        }

        setTogglingGwaType(gwaType);
        try {
            const actor =
                employeeID ||
            ...
            await axios.put(
                `${API_BASE_URL}/api/honors/gwa_printing_status`,
                {
                    gwa_type: gwaType,
            ...
            setGwaPrintStatus((prev) => ({ ...prev, [gwaType]: nextChecked }));
            setSnackbar({
                open: true,
                message: `${gwaType === "overall" ? "Overall" : "Per-semester"} GWA ${nextChecked ? "opened" : "closed"} for students.`,
                severity: "success",
            });
        } catch (err) {
            console.error("Failed to toggle GWA printing status:", err);
            setSnackbar({
                open: true,
                message: "Failed to update GWA printing setting.",
                severity: "error",
            });
        } finally {
            setTogglingGwaType(null);
        }
    };

    const [togglingGwaType, setTogglingGwaType] = useState(null); // 'overall' | 'per_semester' | null
  ...
  <Paper
            ...
            >
                <Typography sx={{ fontWeight: 700, fontSize: "15px" }}>
                    GWA Visibility to Students
                </Typography>

                {gwaPrintStatus.loading ? (
                    ...
                    <Box sx={{ display: "flex", gap: 3, flexWrap: "wrap" }}>
                        <FormControlLabel
                            control={
                                <Switch
                                    checked={gwaPrintStatus.overall}
                                    disabled={togglingGwaType === "overall" || !canEdit}
                                    onChange={(e) =>
                                        handleToggleGwaPrinting("overall", e.target.checked)
                                    }
                                />
                            }
                            label={
                                <Box>
                                    ...
                                    <Typography sx={{ fontSize: 11, color: "text.secondary" }}>
                                        {gwaPrintStatus.overall ? "Open" : "Closed"} — Dashboard, Honors
                                        standing, printing
                                    </Typography>
                                </Box>
                            }
                        ...
                        <FormControlLabel
                            control={
                                <Switch
                                    checked={gwaPrintStatus.per_semester}
                                    disabled={togglingGwaType === "per_semester" || !canEdit}
                                    onChange={(e) =>
                                        handleToggleGwaPrinting("per_semester", e.target.checked)
                                    }
                                />
                            }
                            label={
                                <Box>
                                    ...
                                    <Typography sx={{ fontSize: 11, color: "text.secondary" }}>
                                        {gwaPrintStatus.per_semester ? "Open" : "Closed"} — Grade page,
                                        term printing
                                    </Typography>
                                </Box>
                            }
